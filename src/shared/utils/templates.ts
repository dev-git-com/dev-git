  ...
  generatePackageJson(data: TemplateData): string {
    ...
    builder
      ...
      .setBasicInfo(
        data.projectName,
        "0.0.1",
        "Generated NestJS application from SQL schema",
      ...
      // Database driver
      .addDependency(
        data.databaseConfig.driver,
        this.getDatabaseVersion(data.databaseConfig.typeorm)
      )
      ...
      .addConditionalDependency(data.config.with_ftp, "ftp", "^0.3.10")
      // Dev dependencies
      .addDevDependencies({
        "@nestjs/cli": "^10.0.0",
...
export class ${entityName}Module {}`;
  ...
  private getDatabaseVersion(dbName: string): string {
    ...
    console.log(
      `Using database version for ${dbName}: ${
        versions[dbName as keyof typeof versions]
      }`
    );
